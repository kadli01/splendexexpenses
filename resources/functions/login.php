	...
	if(isset($_POST["bttLogin"]))
		{
			...
			$emailResult = $GLOBALS['db']->prepare("SELECT email FROM users WHERE email = ?");
		    ...
		    $passwordResult = $GLOBALS['db']->prepare("SELECT password FROM users WHERE email = ?");
		    $passwordResult->bindParam(1, $email, PDO::PARAM_STR);
		    $passwordResult->execute();
		    $passwordItem = $passwordResult->fetch();

			$idResult = $GLOBALS['db']->prepare("SELECT user_id FROM users WHERE email = ?");
